...
def dashboard():
    ...
    cursor.execute("SELECT id, mes, ano, fechado FROM meses ORDER BY ano DESC, mes DESC")
    todos_meses = cursor.fetchall()

    mes_id = request.args.get('mes_id', type=int)
    ver_todos = request.args.get('todos', '').strip() == '1'
    ...
    if ver_todos:
        ...
        qtd_saidas = 0
        cursor.execute(
            "SELECT COUNT(*) FROM lancamentos WHERE tipo='entrada'"
        )
        qtd_entradas = cursor.fetchone()[0]
        cursor.execute(
            "SELECT COUNT(*) FROM lancamentos WHERE tipo='saida'"
        )
        qtd_saidas = cursor.fetchone()[0]
        cursor.execute(
            "SELECT l.*, m.mes, m.ano FROM lancamentos l JOIN meses m ON l.mes_id=m.id "
            "ORDER BY l.data DESC, l.id DESC LIMIT 10"
        )
    ...
    if mes_id:
        cursor.execute("SELECT mes, ano, fechado FROM meses WHERE id = ?", (mes_id,))
        ...
        row = cursor.fetchone()
        total_entradas = row[0] if row else 0
        total_saidas = row[1] if row else 0
        cursor.execute(
            "SELECT COUNT(*) FROM lancamentos WHERE mes_id=? AND tipo='entrada'", (mes_id,)
        )
        qtd_entradas = cursor.fetchone()[0]
        cursor.execute(
            "SELECT COUNT(*) FROM lancamentos WHERE mes_id=? AND tipo='saida'", (mes_id,)
        )
        qtd_saidas = cursor.fetchone()[0]
        cursor.execute(
            "SELECT l.*, m.mes, m.ano FROM lancamentos l JOIN meses m ON l.mes_id=m.id "
